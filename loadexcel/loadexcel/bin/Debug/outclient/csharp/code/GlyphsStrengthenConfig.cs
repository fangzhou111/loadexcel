...
{
public class GlyphsStrengthenData
{
/*雕文编号*/
public int ID; 
/*类型*/
...
public partial class GlyphsStrengthenConfig : ConfigBase
{
private static GlyphsStrengthenConfig _instance;
public Dictionary<int, Dictionary<int, GlyphsStrengthenData>> _datas;
public static GlyphsStrengthenConfig Instance
{
get
{
if (_instance == null)
{
_instance = new GlyphsStrengthenConfig();
_instance.Load();
...
private void Load()
{
Stream fs = base.Load("GlyphsStrengthen.bytes");
BinaryReader br = new BinaryReader(fs, System.Text.Encoding.UTF8);
for(int i = 0;i < 45;i++)
{
GlyphsStrengthenData data = new GlyphsStrengthenData();
...
if (_datas == null)
_datas = new Dictionary<int, Dictionary<int, GlyphsStrengthenData>>();
if (!_datas.ContainsKey(data.ID))
{
Dictionary<int, GlyphsStrengthenData> _dic = new Dictionary<int, GlyphsStrengthenData>();
_datas.Add(data.ID, _dic);
}
_datas[data.ID].Add(data.LevelMin,data);
}
br.Close();
fs.Close();
}
public Dictionary<int, GlyphsStrengthenData> Get(int newID)
{
...
public GlyphsStrengthenData Get(int newID,int newLevelMin)
{
if (_datas.ContainsKey(newID) && _datas[newID].ContainsKey(newLevelMin))
return _datas[newID][newLevelMin];
else
return null;
}
}
